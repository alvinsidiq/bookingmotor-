...
<div class="bg-white shadow rounded-lg p-6 max-w-2xl mx-auto">
    <h2 class="text-2xl font-semibold mb-6 text-black">Edit Motorcycle</h2>
    <form action="{{ route('admin.motorcycles.update', $motorcycle) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $motorcycle->name) }}" 
                    class="mt-1 block w-full rounded-md border border-gray-700 bg-white text-black shadow-sm focus:border-black focus:ring focus:ring-gray-400 focus:ring-opacity-50" />
                @error('name')
                    <p class="mt-1 text-sm text-gray-800">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" 
                    class="mt-1 block w-full rounded-md border border-gray-700 bg-white text-black shadow-sm focus:border-black focus:ring focus:ring-gray-400 focus:ring-opacity-50">{{ old('description', $motorcycle->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-gray-800">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @if ($motorcycle->image)
                    <img src="{{ asset('storage/' . $motorcycle->image) }}" alt="Current Image" class="h-12 mt-2 rounded">
                @endif
                @error('image')
                    <p class="mt-1 text-sm text-gray-800">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="rental_rate" id="rental_rate" step="0.01" value="{{ old('rental_rate', $motorcycle->rental_rate) }}" 
                    class="mt-1 block w-full rounded-md border border-gray-700 bg-white text-black shadow-sm focus:border-black focus:ring focus:ring-gray-400 focus:ring-opacity-50" />
                @error('rental_rate')
                    <p class="mt-1 text-sm text-gray-800">{{ $message }}</p>
                @enderror
            ...
            <div class="col-span-2 flex items-center space-x-3 mt-2">
                ...
                <input type="checkbox" name="is_available" id="is_available" value="1" 
                    {{ old('is_available', $motorcycle->is_available) ? 'checked' : '' }} 
